...
- [x] 사용자가 일간 스케줄 보기를 선택했을 경우, 현재 날짜에 해당하는 이벤트 정보가 보여져야 합니다.
- [x] 사용자가 주간 스케줄 보기를 선택했을 경우, 현재 날짜가 속한 주에 해당하는 이벤트 정보가 보여져야 합니다.
...
- [x] 달력에서 이벤트를 클릭했을 경우, 해당 이벤트 상세 페이지(`/event/<EVENT_ID>`)로 이동해야 합니다.
...
- [x] 이벤트를 생성할 수 있는 Form이 보여져야 하고 사용자는 아래 정보를 입력할 수 있어야 합니다.
  - 이벤트 제목
  - 이벤트 설명
  - 이벤트 시작 날짜 및 시간
  - 이벤트 종료 날짜 및 시간
- [x] 위 정보는 모두 필수 정보입니다. 최대한 상식 선에서 스스로 유효성 검사를 실행해 주시기 바랍니다.
- [x] 이벤트가 성공적으로 생성되었을 경우, 메인 달력 페이지로 이동해야 합니다.
- [x] 모든 이벤트는 시작 날짜와 종료 날짜가 같아야 합니다.
- [x] 모든 이벤트는 1시간 단위로만 길이를 조정할 수 있습니다.
- [x] 이벤트는 1시 00분, 2시 00분 등 정시에만 시작하거나 끝날 수 있습니다.
...
- [x] `<EVENT_ID>`에 해당하는 이벤트의 상세 정보를 보여주어야 합니다.
  ...
  - 이벤트 시작 날짜 및 시간
  - 이벤트 종료 날짜 및 시간
- [x] 사용자는 모든 입력 사항에 대해 수정할 수 있습니다.
- [x] 사용자는 이벤트를 삭제할 수 있어야 합니다.
- [x] 만약 유효하지 않은 `<EVENT_ID>`로 접근한다면 유효하지 않은 이벤트라는 정보를 표시해주어야 합니다.
