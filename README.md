...
- [ V ] [Firebase Console](https://console.firebase.google.com)로 이동하세요.
- [ V ] 새 프로젝트를 생성하세요.
- [ V ] Database 서비스 중, **Realtime Database**를 생성하세요. 주의) 🚨 Cloud Firestore가 아닙니다.
- [ V ] 프로젝트 설정에서 본인의 config 정보를 이용하여 `/src/utils/firebase`를 수정하세요.
...
- [ V ] 현재 날짜에 해당하는 달력이 보여야 합니다. (일별로 보기)
- [ V ] 사용자는 일별로 보기, 주간으로 보기 중 하나를 선택할 수 있어야 합니다.
- [ ] 사용자가 일별로 보기를 선택했을 경우, 현재 날짜에 해당하는 이벤트 정보가 보여야 합니다.
- [ ] 사용자가 주간으로 보기를 선택했을 경우, 현재 날짜가 속한 주에 해당하는 이벤트 정보가 보여야 합니다.
- [ V ] 구글 캘린더와 같이 Y축 방향으로는 시간대 정보가 보여야 합니다.
- [ V ] 일별로 보기의 경우, X축 방향으로는 현재 날짜가 보여야 합니다.
- [ V ] 주간으로 보기의 경우, X축 방향으로는 현재 주에 해당하는 날짜가 보여야 합니다.
- [ V ] 이전 날짜/주 혹은 다음 날짜/주로 이동할 수 있는 버튼이 있어야 합니다.
- [ V ] 달력에서 이벤트를 클릭했을 경우, 해당 이벤트 상세 페이지(`/event/<EVENT_ID>`)로 이동해야 합니다.
...
- [ V ] 이벤트를 생성할 수 있는 Form이 보여야 하고 사용자는 아래 정보를 입력할 수 있어야 합니다.
  - 이벤트 제목
  - 이벤트 설명
  - 이벤트 시작 날짜 및 시간
  - 이벤트 종료 날짜 및 시간
- [ V ] 위 정보는 모두 필수 정보입니다. 최대한 상식 선에서 스스로 유효성 검사를 실행해 주시기 바랍니다.
- [ V ] 사용자가 Form을 성공적으로 제출 혹은 저장했을 경우, 메인 달력 페이지로 이동해야 합니다.
- [ V ] 모든 이벤트는 시작일과 종료일이 같아야 합니다.
- [ V ] 모든 이벤트는 1시간 단위로 시간을 조정할 수 있습니다.

#### `/events/<EVENT_ID>` 이벤트 상세 페이지

- [ V ] `<EVENT_ID>`에 해당하는 이벤트의 상세 정보를 보여주어야 합니다.
  - 이벤트 제목
  - 이벤트 설명
  - 이벤트 시작 날짜 및 시간
  - 이벤트 종료 날짜 및 시간
- [ ] 사용자는 모든 입력 사항에 대해 수정할 수 있습니다.
- [ V ] 사용자는 이벤트를 삭제할 수 있어야 합니다.
