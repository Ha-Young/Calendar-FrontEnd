...
export default function CalendarContents({ displayDates, events, selectEvent }) {
  ...
  const days = displayDates.map((date, index) => {
    ...
    for (let hour = 0; hour < 24; hour++) {
      // TODO 아니면 그냥 for 문 말고 events 먼저 순회하고 [] 안에 날짜별 [] 만들고, 해당 시간대에 타이틀 있는지/색칠 해야하는지 표시해서 넘겨주고 render할때 이거 참고해서 그리기만 하면 훨 빠를듯. 이러면 시간복잡도 그냥 7 * 24 + event 개수임.
      // TODO 그냥 for문으로 바꿔서 break 가능하게 바꾸는것도 좋을듯. 지금은 7 * 24 * event 개수가 시간복잡도라 너무 느림. 멸망...
      let isEvent = false;
